...
<div class="container mx-auto px-4 py-6">
    ...
    <form action="{{ route('ledgers.store') }}" method="POST" class="max-w-3xl mx-auto space-y-6">
        @csrf

        <div class="flex items-start">
            <label for="name" class="w-1/3 text-sm font-medium text-gray-900 dark:text-white mt-2">Name</label>
            <div class="w-2/3">
                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="Enter a Name" class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                @error('name')
        ...
        <div class="flex items-start">
            ...
            <div class="w-2/3">
                ...
                class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Enter a Address">{{ old('address') }}</textarea>
                @error('address')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
