        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gradient-to-r from-blue-50 to-gray-50 px-6 py-5 border-b border-gray-200">
            <div class="mb-4 sm:mb-0">
                <div class="flex items-center">
                    <h2 class="text-2xl font-bold text-gray-800">Inward Details</h2>
                    <span class="ml-2 px-2 py-1 text-xs font-semibold bg-blue-100 text-blue-800 rounded-full">
                        #{{ $inward->id }}
                    </span>
                </div>
                <p class="text-sm text-gray-500 mt-1">Created on {{ \Carbon\Carbon::parse($inward->created_at)->format('d M, Y h:i A') }}</p>
            </div>
            <div class="flex space-x-3">
                <button type="button" 
                        onclick="window.location='{{ route('inwards.edit',$inward) }}'"
                ...
                <button onclick="window.location='{{ route('inwards') }}'" 
        ...
        <div class="px-6 py-5">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="text-sm text-gray-500">
                    Showing {{ count($inward->details) }} items
                </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-blue-50 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
                                {{ $detail->quantity }} <span class="text-gray-400 ml-1">{{ $detail->item->unit->abbreviation }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-right">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl ml-auto">
                <div class="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div class="flex items-end">
                        <div class="text-3xl font-bold text-gray-800">{{ $inward->total_quantity }}</div>
                ...
                <div class="bg-green-50 p-5 rounded-xl border border-green-100 shadow-sm">
                    ...
                    <div class="text-3xl font-bold text-green-700">₹{{ number_format($inward->total_amount, 2) }}</div>
                </div>
            </div>
        </div>
