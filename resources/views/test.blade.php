...
<div class="w-full mx-auto bg-white px-4 py-6">
   ...
   <div class="grid grid-cols-1 md:grid-cols-4 border-b gap-4 p-6">
       <div>
            ...
            <input type="date" id="formDate" class="mt-1 block w-full border rounded px-3 py-2" value="{{ \Carbon\Carbon::now()->format('Y-m-d') }}">
        </div>
       <div class="relative">
...
<div id="item-details" style="margin-top: 20px;"></div>
        ...
        if (event.key === 'Enter') {
            ...
            if (barcode.length > 0) {
                fetch(`/item-by-barcode/${barcode}`)
                    ...
                    .then(data => {
                        let box = document.getElementById('itemsTableBody');
                        
                    if (data.error) {
                            box.innerHTML = `noDataRow `;
                        } else {
                            box.innerHTML = `
                             <tr>
                                  <td class="px-3 py-3" ></td>
                                     <td class="px-3 py-3" >${data.barcode}</td>
                                        <td class="px-3 py-3" >${data.name}</td>
                                           <td class="px-3 py-3" >${data.category.name}</td>
                                              <td class="px-3 py-3" >${data.hsn_code}</td>
                                                 <td class="px-3 py-3" >${data.current_stock}</td>
                                                    <td class="px-3 py-3" >${data.unit.abbreviation }</td>
                                                     <td class="px-3 py-3" >${data.price}</td>
                                                    </tr> `;
                        }
...
<script>
    (function() {
        const allItems = @json($items); // your items preloaded in controller
        ...
        async function fetchLedgers(query) {
            if (!query) return [];
            try {
                const response = await fetch(`/api/ledgers?q=${encodeURIComponent(query)}`);
        ...
        function updateSaveButton() {
            const canSave = selectedItems.length > 0 && selectedItems.every(i => i.qty >= 1 && i.qty <= i.current_stock) && selectedLedger !== null;
        ...
        function renderItems() {
            ...
            selectedItems.forEach((item, index) => {
                ...
                tdCategory.textContent = item.category_name;
        ...
        barcodeInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                ...
                const foundItem = allItems.find(i => i.barcode === barcode);
                if (!foundItem) {
